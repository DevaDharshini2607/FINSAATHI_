...
const ImageSlider = () => {
    ...
    return (
        <div className="relative w-full h-[400px] md:h-[500px] overflow-hidden group bg-gray-900">
            {slides.map((slide, index) => (
                <div
                    key={slide.id}
                    className={`absolute inset-0 transition-all duration-1000 ease-in-out ${index === current ? 'opacity-100 z-10' : 'opacity-0 z-0'
                        }`}
                >
                    ...
                    <div className="absolute inset-0 overflow-hidden">
                        <img
                            src={slide.image}
                            alt={slide.title}
                            className={`w-full h-full object-cover transition-transform duration-[6000ms] ease-out ${index === current ? 'scale-110' : 'scale-100'
                                }`}
                        />
                        ...
                        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
                            <div className={`max-w-2xl transform transition-all duration-1000 delay-300 ${index === current ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
                                ...
                                <div className="flex items-center space-x-2 mb-4">
                                    ...
                                    <span className={`text-sm font-bold tracking-wider uppercase ${slide.color}`}>
                                        Featured Capability
                                    </span>
                                </div>
                                ...
                                <p className="text-lg md:text-xl text-gray-200 leading-relaxed border-l-4 border-brand-blue pl-6 bg-black/20 backdrop-blur-sm py-4 rounded-r-lg">
                                    {slide.desc}
                                </p>
                                <a href="#features" className="inline-block mt-8 px-8 py-3 bg-white text-brand-dark font-semibold rounded-lg hover:bg-brand-light-blue transition-colors duration-300 shadow-lg">
                                    Explore Feature
                                </a>
                            </div>
                        </div>
            ...
            <button
                onClick={prevSlide}
                className="absolute left-4 top-1/2 -translate-y-1/2 z-20 p-3 bg-white/10 hover:bg-white/20 rounded-full text-white backdrop-blur-md border border-white/20 opacity-0 group-hover:opacity-100 transition-all duration-300 transform -translate-x-4 group-hover:translate-x-0"
            >
                <ChevronLeft className="w-6 h-6" />
            </button>
            <button
                onClick={nextSlide}
                className="absolute right-4 top-1/2 -translate-y-1/2 z-20 p-3 bg-white/10 hover:bg-white/20 rounded-full text-white backdrop-blur-md border border-white/20 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-4 group-hover:translate-x-0"
            >
            ...
            <div className="absolute bottom-8 left-0 right-0 z-20 flex justify-center space-x-3">
                {slides.map((_, idx) => (
                    <button
                        ...
                        className={`transition-all duration-300 rounded-full h-1.5 ${idx === current ? 'w-12 bg-brand-blue' : 'w-6 bg-white/40 hover:bg-white/60'
                            }`}
